...
class ScrapearGMaps:
  ...
  def get_name(self):
    try:
      #extract the headert-title element text, and quit all the weird characters using cleanString method DUwDvf fontHeadlineLarge
      #return self.cleanString(str(self.driver.find_element_by_xpath("//h1[contains(@class,'header-title')]").text))
      return self.cleanString(str(self.driver.find_element_by_xpath("//h1[contains(@class,'fontHeadlineLarge')]").text))
    except:
  ...
  def scrape(self, times = 20):
    try:
      ...
      for i in range(0,times):
        ...
        print("polo")
        if place == None:
          print("No he encontrado el elemento"+str(i))
          continue
  ...
  def searchNextElement(self,element):#hfpxzc V0h1Ob-haAclf 
    ...
    while len(self.driver.find_elements(By.CLASS_NAME, elementClass)) < element:
      try:
        #estoy probando con todas las clases a ver cual es a la que le tengo que aplicar el ArrowDown
        #porque esta petando porque el panel no esta descendiendo, por lo que no se cargan mas lugares y no llega a los 20
        # con peta porque no se mueve:
        #  - "Yr7JMd-pane-content cYB2Ge-oHo7ed", "m6QErb DxyBCb cYB2Ge-oHo7ed cYB2Ge-ti6hGc ecceSd", "m6QErb WNBkOb", "section-scrollbox","Yr7JMd-pane Yr7JMd-pane-visible"
        for htmlElement in self.driver.find_elements(By.CLASS_NAME, "scroll"):
          for i in range(7*element):
            htmlElement.send_keys(Keys.ARROW_DOWN)
            self.naturalWait(0)
          for i in range(7*element):
            htmlElement.send_keys(Keys.PAGE_DOWN)
            self.naturalWait(0)
        #self.driver.find_element(By.CLASS_NAME, "m6QErb DxyBCb cYB2Ge-oHo7ed cYB2Ge-ti6hGc ecceSd").send_keys(Keys.ARROW_DOWN)
        # self.driver.find_element(By.CLASS_NAME, "m6QErb WNBkOb").send_keys(Keys.ARROW_DOWN)
        #self.driver.find_elements(By.CLASS_NAME, "section-scrollbox")[1].send_keys(Keys.ARROW_DOWN)
      except:
        print("Ha petado en el bucle xd")
        break
    #print(element,len(self.driver.find_elements(By.CLASS_NAME, elementClass)))
    self.naturalWait(1)
    
    #if the searched element is bigger than the number of places, you didnt found the searched place
