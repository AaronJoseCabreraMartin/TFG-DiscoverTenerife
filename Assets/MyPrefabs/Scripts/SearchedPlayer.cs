...
public class SearchedPlayer : MonoBehaviour
{
    ...
    public void sendInvitation(){
        ...
        Debug.Log("searchedPlayerData_.uid = " + searchedPlayerData_["uid"] 
                + " current user uid = " + firebaseHandler.firebaseHandlerInstance_.currentUser_.getUid());
        
        Debug.Log("searchedPlayerData_.name = " + searchedPlayerData_["name"] 
                + " current user name = " + firebaseHandler.firebaseHandlerInstance_.currentUser_.getDisplayName());
        
        if(searchedPlayerData_["uid"] == firebaseHandler.firebaseHandlerInstance_.currentUser_.getUid()){
            toastMessage_.GetComponent<toastMessage>().makeAnimation("You can't send a friendship invitation to yourself.",new Color32(255,0,0,255), 5);
            ...
            toastMessage_.GetComponent<toastMessage>().makeAnimation("You don't have access to internet, try it again later",new Color32(255,0,0,255), 5);
        }
    }
    ...
    public void resultOfTheSending(string result){
        ...
        if(result == "failed"){
            ...
            toastMessage_.GetComponent<toastMessage>().makeAnimation("Error, the petition couldn't be sended, please try it again",new Color32(255,0,0,255), 5);
        }else if(result == "repeated"){
            //error, {usuario} ya tiene una peticion de amistad tuya pendiente 
            toastMessage_.GetComponent<toastMessage>().makeAnimation("Error, the user "+searchedPlayerData_["name"]+" already have a friendship invitation of you",new Color32(255,145,15,255), 5);
        }else if(result == "sended"){
            //toast diciendo OK!
            toastMessage_.GetComponent<toastMessage>().makeAnimation("You have sended a friendship invitation to "+searchedPlayerData_["name"]+" successfully", new Color32(76,175,80,255), 5);
